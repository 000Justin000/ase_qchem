...
ertol = 1.0e-10
#------------------------------------------------
nrot = int(sys.argv[6])             # number of angular discret
#------------------------------------------------
...
for diangle in diangles_loc:
    ...
    #----------------------------------------
    prefix = "dihedrals_"+"{:02.0f}".format(rb1[0])
                         +"{:02.0f}".format(rb1[1])
                         +"{:02.0f}".format(rb1[2])
                         +"{:02.0f}".format(rb1[3])
                         +"{:+04.0f}".format(diangle[0])
    #----------------------------------------
    ...
    if ((asemol is not None) and (E is not None)):
        energies_loc.append((diangle[0], E))
        ase.io.write(dir_name+"/" + prefix +".xyz", asemol)
        print("theta1: %+04.0f, energy: %15.7f" % (diangle[0], E))
        sys.stdout.flush()
    else:
        print("theta1: %+04.0f, optimization failed" % (diangle[0]))
        sys.stdout.flush()
    #----------------------------------------
